#!/bin/bash
# Lot 5's P2 on A2-BN (cnn_gn_cifar_bn): the operational preconditioner of all five Fisher modes against the exact
# references, at the five checkpoints of cnn_gn_cifar_bn/diag seed 0, alongside the P1 and P1-py rungs it
# has to be compared with. docs/reports/plan_exp_lot5.md §3.5; plan_exp_draft.md §9 (lot 5), §13.
# Submit from the repository root:  sbatch fisher_ref/slurm/p2_operational_a2_bn.sh
#
# Hand-written, like every job in this directory (fisher_ref/slurm/README.md).

#SBATCH --account=def-msh-ab
#SBATCH --job-name=fisher_ref_p2_a2_bn
...
# Sizing. P = 24 458.
#
#   device    one P x P fp64 accumulator (4.8 GB) during the reference build, and -- separately in
...
#             traversal starts.
#   host      10 P x P folds (47.9 GB) + the assembled matrix + a half pair during the
#             intervals + the per-block rearrangements. lot 3 measured 80.7 GB on this model; 128G is that plus headroom.
#   time      lot 3 measured ~60 min per checkpoint on this model at this probe count (job 21276895).
...
#             cost instead of three times it. ~100 min per checkpoint, ~8.3 h for five; --time=16:00:00 is that plus headroom.
...
# This is the model whose BatchNorm2d layers the optimizer DOES hook, so it is the one
# where the operational reading of a normalisation layer can be compared with its exact
# block -- the GroupNorm variant's norms take the identity instead (plan_exp_lot5.md §0.7).
#
...
#   benchmarks/outputs/cifar10/cnn_gn_cifar_bn/diag/ckpt_{0,0.01,0.1,0.5,1}.pt
# ---------------------------------------------------------------------------------------------
...
P2_MODEL="${P2_MODEL:-cnn_gn_cifar_bn}"
