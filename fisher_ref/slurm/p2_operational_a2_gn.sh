#!/bin/bash
# Lot 5's P2 on A2 (cnn_gn_cifar, GroupNorm): the operational preconditioner of all five Fisher modes against the exact
# references, at the five checkpoints of cnn_gn_cifar/diag seed 0, alongside the P1 and P1-py rungs it
# has to be compared with. docs/reports/plan_exp_lot5.md §3.5; plan_exp_draft.md §9 (lot 5), §13.
# Submit from the repository root:  sbatch fisher_ref/slurm/p2_operational_a2_gn.sh
#
# Hand-written, like every job in this directory (fisher_ref/slurm/README.md).

#SBATCH --account=def-msh-ab
#SBATCH --job-name=fisher_ref_p2_a2_gn
...
#   time      lot 3 measured ~60 min per checkpoint on this model at this probe count (job 21276894).
...
P2_MODEL="${P2_MODEL:-cnn_gn_cifar}"
