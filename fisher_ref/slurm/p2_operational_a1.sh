#!/bin/bash
# Lot 5's P2 on A1 (mlp_ln_mnist): the operational preconditioner of all five Fisher modes against the exact
# references, at the five checkpoints of mlp_ln_mnist/diag seed 0, alongside the P1 and P1-py rungs it
# has to be compared with. docs/reports/plan_exp_lot5.md §3.5; plan_exp_draft.md §9 (lot 5), §13.
# Submit from the repository root:  sbatch fisher_ref/slurm/p2_operational_a1.sh
#
# Hand-written, like every job in this directory (fisher_ref/slurm/README.md).

#SBATCH --account=def-msh-ab
#SBATCH --job-name=fisher_ref_p2_a1
...
# Sizing. P = 26 634.
#
#   device    one P x P fp64 accumulator (5.7 GB) during the reference build, and -- separately in
...
#             traversal starts.
#   host      8 P x P folds (45.4 GB) + the assembled matrix + a half pair during the
#             intervals + the per-block rearrangements. eight folds plus, at features.0, a 5.05 GB block and a 5.05 GB rearrangement per half.
#   time      lot 3 measured ~8 min (lot 2, no folds) per checkpoint on this model at this probe count (job 21128864).
...
#             cost instead of three times it. ~55 min per checkpoint, ~4.6 h for five; --time=12:00:00 is that plus headroom.
...
#
# --probes/--batch/--folds are NOT lot 2's 55 000: eight folds of whole micro-batches need the probe count to
# divide folds x batch, and 55 000 % 2 000 != 0. So A1's P1 rows here are not comparable
# with lot 2's, which is stated rather than assumed. Eight folds instead of ten because
# this is the campaign's largest P and its first layer alone is 25 120 of it.
#
# --rho-max-p 30000 keeps features.0 IN: it is 25 120 parameters and carries 63 % of
# tr(F), and lot 2's first A1 job skipped M5 there, which needed a second job to repair
# (p1_features0_a1.sh). One Cholesky of 25 120^2 fp64 is 30 s, MEASURED there; five
# alphas x two sources is ~300 s per checkpoint. --noise-rho-max-p 8192 keeps it out of
# the 20 partitions' intervals, where it would be 40 more Choleskys.
#
# A1 is also where two known defects would bite: its first Linear's input factor is
# 785 x 785 of rank 646 (130 of MNIST's pixels are identically zero), which is the matrix
# cuSOLVER's eigh raised _LinAlgError on and killed two campaign-1 runs. The ekfac and
# tekfac re-warms run straight into it; _eigh_utils.py::eigenbasis is the fix that has
# to hold.
#
...
P2_MODEL="${P2_MODEL:-mlp_ln_mnist}"
