#!/bin/bash
# Lot 3 P1 on A2-BN (cnn_gn_cifar_bn: BatchNorm2d in eval, with the checkpoint's running
# statistics): every structure of the zoo against the exact references, the weight-sharing
# decomposition and per-layer fold intervals, at the five checkpoints of cnn_gn_cifar_bn/diag seed 0
# (the manifest resolves the bench to cnn_gn_cifar with norm=bn).
# docs/reports/plan_exp_lot3.md §0.11; plan_exp_draft.md §9 (lot 3), §13.
# Submit from the repository root:  sbatch fisher_ref/slurm/p1_structural_a2_bn.sh
#
# Hand-written, like every job in this directory (fisher_ref/slurm/README.md).

#SBATCH --account=def-msh-ab
#SBATCH --job-name=fisher_ref_p1_a2_bn
...
# ---------------------------------------------------------------------------------------------
# Sizing: identical to p1_structural_a2_gn.sh (same P = 24 458, same T; BN carries 2C affine
# parameters like GroupNorm). BN is hooked by AdaFisherMulti, so diag_py is read here, not on GN.
#
...
P1_MODEL="${P1_MODEL:-cnn_gn_cifar_bn}"
