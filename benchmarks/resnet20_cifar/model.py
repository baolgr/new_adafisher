"""B2 of ``plan_exp_draft.md`` §4: ResNet-20 for CIFAR-10 (``papers/resnet_1512.03385.pdf`` §4.2),
written from the paper. 269 722 parameters — the paper's own "0.27M" for its ``n = 3`` network.
...
(a projection shortcut) would add ~2 000 parameters and no longer match the published figure.

Hook safety (``plan_exp_step1.md`` §4, rule 1): the residual add is ``out = out + shortcut``,
never ``+=``, and every ``ReLU`` is ``inplace=False``.
"""
