"""CCT-2/3x2 for CIFAR-10 (Hassani et al., *Escaping the Big Data Paradigm with Compact
Transformers*, arXiv:2104.05704). **283 723 parameters** (counted) — the paper's own "0.28M" for
its smallest variant, and one of AdaFisher's own models
...
the paper's stated small-model width ``d = 128``, 2 heads and MLP ratio 1 (Table 2's "CCT-2" row).

Two components distinguish CCT from a plain ViT, and are the reason this model is here:
...
Deviations from the authors' reference implementation, stated rather than discovered: stochastic
depth (0.1 in their CIFAR configuration) is **not** implemented, because it randomizes the
computational graph per sample and a curvature-fidelity bench has no use for that; dropout and
...
``LayerNorm``, so every residual add is ``x = x + ...``, never ``+=``. And every learnable
projection must live inside one of those four module types, so attention exposes its projection as
a single hookable ``nn.Linear`` named ``qkv`` rather than using ``nn.MultiheadAttention``, whose
``in_proj_weight`` is a raw ``Parameter`` no hook would ever see. The learned position embedding is
a raw ``Parameter`` belonging to no hooked module, by construction.
"""
