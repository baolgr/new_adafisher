...
Architectures" + Table 1), written from the paper rather than imported from torchvision or timm.
**23 520 842 parameters** at 10 classes (counted), across 107 hooked modules.

Input: one 32x32 RGB image. Output: 10 class logits.

§4.2's CIFAR stem is a single stride-1 ``3x3`` convolution on the 32x32 input — not the
``7x7``/stride-2 plus max-pool of the ImageNet variant, which would collapse 32x32 to 8x8 before
the first residual block. ``benchmarks/models/resnet50_imagenet/model.py`` is that variant, and
imports the bottleneck block from here rather than copying it.

Block layout is Table 1's "50-layer" column, ``(3, 4, 6, 3)`` bottlenecks of widths 64/128/256/512
with ``expansion = 4``. BatchNorm goes "right after each convolution and before activation" (§3.4),
the shortcut is a projection (``1x1`` convolution + BatchNorm, option B) exactly when the shortcut
has to change shape, and weights are initialized as in §3.4.
...
The residual add is therefore ``out = out + identity``, never ``+=``, and every ``ReLU`` is
``inplace=False`` — which is why ``torchvision.models.resnet50``, whose ``Bottleneck.forward``
does ``out += identity``, is not reused.
"""
