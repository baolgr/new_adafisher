...
# ----------------------------------------------------------------------------------------------
# COMPLETING the seed table: the same seeds, but all seven arms.
#
# Why this exists. ``V0_SEEDS`` above runs two arms, because plan_exp_draft_v0.md §3.5 samples two
# trajectories. That was the right scope for the *drift* question, and it left a different hole:
# ``kfac``, ``ekfac``, ``tkfac``, ``tekfac`` and ``adam`` exist at **seed 0 and nowhere else**, on
# every model. Measured from the bridge before this job was written::
#
#     mlp_ln_mnist    diag/adamw seeds=[0,1,2,3,4]   the other five arms seeds=[0]
#     cnn_gn_cifar    diag/adamw seeds=[0,1,2,3,4]   the other five arms seeds=[0]
#     vit_micro_cifar diag/adamw seeds=[0,1,2,3,4]   the other five arms seeds=[0]
#     cct_2_3x2_cifar diag/adamw seeds=[0,1,2]       the other five arms seeds=[0]
#     resnet20_cifar  diag/adamw seeds=[0,1,2]       the other five arms seeds=[0]
#     cnn_gn_cifar_bn every arm  seeds=[0]
#     mnist_autoencoder every arm seeds=[0]
#
# So every statement this repository makes that *compares the five modes to each other* rests on
# one draw. That includes the primary bench's headline finding (all five Fisher modes freeze at
# MSE ~= 0.7085 while adam descends to 0.4836) and every per-mode ranking in lots 3 and 5.
#
# Two models are added that ``V0_SEEDS`` never covered at all:
#   * ``cnn_gn_cifar_bn`` -- A2's BatchNorm variant, one of the four regime-A models lot 5 runs on.
#     It is the SAME bench as ``cnn_gn_cifar`` with ``--norm bn``; only the output directory
#     differs, which is why this table is keyed by directory and carries the bench separately.
#   * ``mnist_autoencoder`` -- the project's primary bench, and the one carrying the stall finding.
#
# Regime B stays at 3 seeds (1-2 here) and regime A at 5 (1-4): this job completes the table the
# plan asks for, it does not enlarge it.
#
# Cost, exact by construction of the WCT protocol (7 x T_diag per (model, seed), with the measured
# T_diag of the WALLTIME table above): 4.7 h of MIG slice for the whole completion.
#
# One consequence to be explicit about: ``diag`` and ``adamw`` are re-run, because the reference
# arm is what derives the budget in-process and because a run directory should carry one report
# covering all of its arms. Under ``--lr-schedule nominal`` at a fixed seed those two arms are
# bit-reproducible -- the campaign-1 audit found the ``diag`` arm identical step for step across
# two campaigns a day apart -- so the overwrite is idempotent, and any disagreement is itself
# worth seeing.
SEED_COMPLETION_TIME_BASIS = (
    "MEASURED. 7 x T_diag x {n} seeds = {train}, from the T_diag of the WALLTIME table above,"
    " plus the ~3 min of eval and setup a grouped job of this model measured end to end."
)
#: directory under ``outputs/seeds/`` -> (bench, seeds, extra CLI flags, --time, T_diag in seconds)
SEED_COMPLETION = {
    "mnist_autoencoder": ("mnist_autoencoder", [1, 2, 3, 4], "", "00:20:00", 13.7),
    "mlp_ln_mnist":      ("mlp_ln_mnist",      [1, 2, 3, 4], "", "00:25:00", 21.6),
    "cnn_gn_cifar":      ("cnn_gn_cifar",      [1, 2, 3, 4], "", "00:40:00", 46.0),
    "cnn_gn_cifar_bn":   ("cnn_gn_cifar",      [1, 2, 3, 4], "--norm bn", "00:40:00", 46.0),
    "vit_micro_cifar":   ("vit_micro_cifar",   [1, 2, 3, 4], "", "00:55:00", 67.4),
    "cct_2_3x2_cifar":   ("cct_2_3x2_cifar",   [1, 2],       "", "01:30:00", 237.5),
    "resnet20_cifar":    ("resnet20_cifar",    [1, 2],       "", "02:10:00", 360.3),
}
...
SEED_COMPLETION_BODY = """# Completes the extra-seed table for {label}: all {n_arms} arms, seeds {seedlist}, one job.
#
...
# --lr-schedule nominal, --reference-arm {reference}, --max-epoch-factor 3, the same checkpoint
...
#
# Writes to benchmarks/outputs/seeds/{label}/seed<n>/ -- ungrouped, its axis being the seed.
...
run_one () {{  # $1 = seed
  ...
  python -m benchmarks.models.{model}.bench \\
    ...
    --reference-arm {reference} \\
    --max-epoch-factor 3 \\
    --lr-schedule {lr_schedule} \\{extra}
    --num-workers {workers} \\
    --no-allow-download \\
    --data-root "$DATA_ROOT" \\
    --checkpoints 0,0.01,0.1,0.5,1 \\
    --no-plot \\
    --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/seeds/{label}/seed$1" \\
  || FAILED+=("{label}:seed$1")
}}
...
echo "all {label} seed runs completed"
"""
...
def write_seed_completion(benches) -> None:
    """One job per model for the all-arm completion of the seed table (see SEED_COMPLETION)."""
    for label, (model, seeds, extra, time, t_diag) in SEED_COMPLETION.items():
        bench = benches[model]
        invocations = "\n".join(f"run_one {seed}" for seed in seeds)
        train_s = len(bench.arms) * t_diag * len(seeds)
        filename = f"train_seeds_{label}.sh"
        write(
            filename,
            header(
                bench,
                title=(f"{bench.title()}{' (' + extra + ')' if extra else ''}: "
                       f"the extra-seed table completed to all "
                       f"{len(bench.arms)} arms, seeds {', '.join(map(str, seeds))}. "
                       f"Writes to benchmarks/outputs/seeds/{label}/seed<n>/."),
                filename=filename, job_name=f"seeds_{label}", time=time,
                time_basis=SEED_COMPLETION_TIME_BASIS.format(
                    n=len(seeds), train=f"{train_s / 60:.1f} min of training"),
            )
            + SEED_COMPLETION_BODY.format(
                label=label, model=model, arms=" ".join(bench.arms),
                n_arms=len(bench.arms), seedlist=", ".join(map(str, seeds)),
                reference=REFERENCE_ARM, lr_schedule=V0_SEEDS_LR_SCHEDULE,
                workers=workers_of(bench), invocations=invocations,
                extra=f"\n    {extra} \\" if extra else "",
            ),
        )
