#!/bin/bash
# CIFAR-10, ResNet-50, all 7 arms in one job. Writes to benchmarks/outputs/cifar10/resnet50_cifar/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/cifar10/train_resnet50_cifar_all.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=resnet50_cifar_all
...
# --time: MEASURED. 7 x T_diag = 7 x 2822.8 s = 5.49 h of training; +6% validation and ~40 s setup = ~5.9 h. The longest job of the campaign.
...
#
# The per-arm alternative is benchmarks/slurm/cifar10/train_resnet50_cifar_<arm>.sh, one job each; use it if you
...
python -m benchmarks.resnet50_cifar.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/cifar10/resnet50_cifar"
