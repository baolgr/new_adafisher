#!/bin/bash
# CIFAR-10, CNN 16-32-64 + GroupNorm, arm=diag (WCT reference arm). Writes to benchmarks/outputs/cifar10/cnn_gn_cifar/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/cifar10/train_cnn_gn_cifar_diag.sh
...
# --time: MEASURED. T_diag = 46.0 s; 7 arms = 322 s of training, ~6.5 min of job.
...
#   python -c "import json;print(json.load(open('benchmarks/outputs/cifar10/cnn_gn_cifar/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.models.cnn_gn_cifar.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/cifar10/cnn_gn_cifar/diag"
