#!/bin/bash
# CIFAR-10, ViT-micro d=32, all 7 arms in one job. Writes to benchmarks/outputs/cifar10/vit_micro_cifar/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/cifar10/train_vit_micro_cifar_all.sh
...
# --time: MEASURED. 7 x T_diag = 7 x 67.4 s = 472 s of training.
...
#
# The per-arm alternative is benchmarks/slurm/cifar10/train_vit_micro_cifar_<arm>.sh, one job each; use it if you
...
python -m benchmarks.models.vit_micro_cifar.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/cifar10/vit_micro_cifar"
