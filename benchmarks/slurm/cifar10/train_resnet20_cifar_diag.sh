#!/bin/bash
# CIFAR-10, ResNet-20, arm=diag (WCT reference arm). Writes to benchmarks/outputs/cifar10/resnet20_cifar/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/cifar10/train_resnet20_cifar_diag.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=resnet20_cifar_diag
...
# --time: MEASURED. T_diag = 360.3 s (resnet20_cifar_all-20851634.out); 7 arms = 42 min of training, ~45 min of job.
...
#   python -c "import json;print(json.load(open('benchmarks/outputs/cifar10/resnet20_cifar/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.models.resnet20_cifar.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/cifar10/resnet20_cifar/diag"
