#!/bin/bash
# CIFAR-10, ViT-S/4, all 7 arms in one job. Writes to benchmarks/outputs/cifar10/vit_small_cifar/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/cifar10/train_vit_small_cifar_all.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=vit_small_cifar_all
...
# --time: MEASURED. 7 x T_diag = 7 x 987.8 s = 1.92 h of training; +6% validation and ~40 s setup = ~2.1 h.
...
#
# The per-arm alternative is benchmarks/slurm/cifar10/train_vit_small_cifar_<arm>.sh, one job each; use it if you
...
python -m benchmarks.models.vit_small_cifar.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/cifar10/vit_small_cifar"
