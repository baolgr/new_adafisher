#!/bin/bash
# CIFAR-10, ViT-micro d=32, arm=diag (WCT reference arm). Writes to benchmarks/outputs/cifar10/vit_micro_cifar/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/cifar10/train_vit_micro_cifar_diag.sh
...
#   python -c "import json;print(json.load(open('benchmarks/outputs/cifar10/vit_micro_cifar/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.models.vit_micro_cifar.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/cifar10/vit_micro_cifar/diag"
