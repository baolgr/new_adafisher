#!/bin/bash
# CIFAR-10, ResNet-20, arm=diag (WCT reference arm). Writes to benchmarks/outputs/resnet20_cifar/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/train_resnet20_cifar_diag.sh
...
# --time: MEASURED. Worst arm tekfac at 20.76 ms/step; 351 steps/epoch x 50 epochs = 17 550 steps -> ~6.1 min of training, ~6.3 min with validation.
...
#   python -c "import json;print(json.load(open('benchmarks/outputs/resnet20_cifar/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.resnet20_cifar.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/resnet20_cifar/diag"
