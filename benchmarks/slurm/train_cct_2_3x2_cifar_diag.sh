#!/bin/bash
# CIFAR-10, CCT-2/3x2, arm=diag (WCT reference arm). Writes to benchmarks/outputs/cct_2_3x2_cifar/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/train_cct_2_3x2_cifar_diag.sh
...
# --time: MEASURED. Worst arm ekfac at 13.47 ms/step; 351 steps/epoch x 50 epochs = 17 550 steps -> ~3.9 min of training, ~4.1 min with validation.
...
#   python -c "import json;print(json.load(open('benchmarks/outputs/cct_2_3x2_cifar/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.cct_2_3x2_cifar.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/cct_2_3x2_cifar/diag"
