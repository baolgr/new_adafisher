#!/bin/bash
# ImageNet-1K, ResNet-50, arm=diag (WCT reference arm). Writes to benchmarks/outputs/imagenet/resnet50_imagenet/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/imagenet/train_resnet50_imagenet_diag.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=resnet50_imagenet_diag
#SBATCH --gpus=h100:1
...
# --time: ESTIMATED. ~1250 img/s fp32 on one H100 = ~8.5 h for 30 epochs.
...
#   python -c "import json;print(json.load(open('benchmarks/outputs/imagenet/resnet50_imagenet/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.models.resnet50_imagenet.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/imagenet/resnet50_imagenet/diag"
