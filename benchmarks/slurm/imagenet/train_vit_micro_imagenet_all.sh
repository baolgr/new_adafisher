#!/bin/bash
# ImageNet-1K @32px, ViT-micro d=32, all 7 arms in one job. Writes to benchmarks/outputs/imagenet/vit_micro_imagenet/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/imagenet/train_vit_micro_imagenet_all.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=vit_micro_imagenet_all
...
# --time: ESTIMATED. 7 x ~2 h, data-loader bound.
...
#
# The per-arm alternative is benchmarks/slurm/imagenet/train_vit_micro_imagenet_<arm>.sh, one job each; use it if you
...
python -m benchmarks.models.vit_micro_imagenet.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/imagenet/vit_micro_imagenet"
