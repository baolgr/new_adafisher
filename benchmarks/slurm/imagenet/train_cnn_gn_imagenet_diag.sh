#!/bin/bash
# ImageNet-1K @32px, CNN 16-32-64 + GroupNorm, arm=diag (WCT reference arm). Writes to benchmarks/outputs/imagenet/cnn_gn_imagenet/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/imagenet/train_cnn_gn_imagenet_diag.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=cnn_gn_imagenet_diag
...
#   python -c "import json;print(json.load(open('benchmarks/outputs/imagenet/cnn_gn_imagenet/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.models.cnn_gn_imagenet.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/imagenet/cnn_gn_imagenet/diag"
