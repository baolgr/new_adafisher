#!/bin/bash
# ImageNet-1K @32px, ViT-micro d=32, arm=diag (WCT reference arm). Writes to benchmarks/outputs/imagenet/vit_micro_imagenet/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/imagenet/train_vit_micro_imagenet_diag.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=vit_micro_imagenet_diag
...
# Reference arm (plan_lot8.md §0.7): a fixed 40-epoch run whose measured elapsed time is
# every other arm's wall-clock budget. Read it back from the run's manifest.json:
#   python -c "import json;print(json.load(open('benchmarks/outputs/imagenet/vit_micro_imagenet/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.models.vit_micro_imagenet.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/imagenet/vit_micro_imagenet/diag"
