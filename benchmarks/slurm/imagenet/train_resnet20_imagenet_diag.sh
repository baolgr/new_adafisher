#!/bin/bash
# ImageNet-1K @32px, ResNet-20, arm=diag (WCT reference arm). Writes to benchmarks/outputs/imagenet/resnet20_imagenet/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/imagenet/train_resnet20_imagenet_diag.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=resnet20_imagenet_diag
...
# --time: ESTIMATED. 27.9x resnet20_cifar's samples/epoch, 40 epochs.
...
#   python -c "import json;print(json.load(open('benchmarks/outputs/imagenet/resnet20_imagenet/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.models.resnet20_imagenet.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/imagenet/resnet20_imagenet/diag"
