#!/bin/bash
# ImageNet-1K @32px, CNN 16-32-64 + GroupNorm, all 7 arms in one job. Writes to benchmarks/outputs/imagenet/cnn_gn_imagenet/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/imagenet/train_cnn_gn_imagenet_all.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=cnn_gn_imagenet_all
...
#
# The per-arm alternative is benchmarks/slurm/imagenet/train_cnn_gn_imagenet_<arm>.sh, one job each; use it if you
...
python -m benchmarks.models.cnn_gn_imagenet.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/imagenet/cnn_gn_imagenet"
