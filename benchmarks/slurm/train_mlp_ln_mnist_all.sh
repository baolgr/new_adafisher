#!/bin/bash
# MNIST, MLP 784-32-32-10 + LayerNorm, all 7 arms in one job. Writes to benchmarks/outputs/mlp_ln_mnist/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/train_mlp_ln_mnist_all.sh
...
#
# The per-arm alternative is benchmarks/slurm/train_mlp_ln_mnist_<arm>.sh, one job each; use it if you
...
python -m benchmarks.mlp_ln_mnist.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/mlp_ln_mnist"
