#!/bin/bash
# MNIST, MLP 784-32-32-10 + LayerNorm, all 7 arms in one job. Writes to benchmarks/outputs/mnist/mlp_ln_mnist/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/mnist/train_mlp_ln_mnist_all.sh

#SBATCH --account=def-msh-ab
#SBATCH --job-name=mlp_ln_mnist_all
...
# --time: MEASURED. 7 x T_diag = 7 x 21.6 s = 151 s of training.
...
# protocol in-process): the reference arm runs 20 epochs unbudgeted, its measured elapsed
...
#
# The per-arm alternative is benchmarks/slurm/mnist/train_mlp_ln_mnist_<arm>.sh, one job each; use it if you
...
python -m benchmarks.mlp_ln_mnist.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/mnist/mlp_ln_mnist"
