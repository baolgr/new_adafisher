#!/bin/bash
# MNIST auto-encoder 784-1000-500-250-30, arm=diag (WCT reference arm). Writes to benchmarks/outputs/mnist_autoencoder/diag/.
...
# Submit from the repository root:  sbatch benchmarks/slurm/train_mnist_autoencoder_diag.sh
...
# --time: UNMEASURED. 2.8 M params, 110 steps/epoch at batch 500; MNIST is CPU-bound at this size.
...
#   python -c "import json;print(json.load(open('benchmarks/outputs/mnist_autoencoder/diag/manifest.json'))['arms']['diag']['total_s'])"
...
python -m benchmarks.mnist_autoencoder.bench \
  ...
  --output-dir "$SLURM_SUBMIT_DIR/benchmarks/outputs/mnist_autoencoder/diag"
